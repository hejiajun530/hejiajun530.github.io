<template>
  <div>
    ...
    >
      ...
      >
        <div
          class="alert-title d-flex jc-between ai-center"
        ...
        >
          ...
          <div>{{text.title}}</div>
          <div
            class="alert-title-close d-flex jc-center ai-center"
            @click="handleClickOk('close')"
          >×</div>
        </div>
        <div class="alert-content">{{text.content}}</div>
        <div class="alert-bottom d-flex jc-end ai-center">
          <button
    ...
    handleMouseMoveAlert(e) {
      ...
      if (_self.moveFlag) {
        ...
        if (_self.moveX < 0) {
          ...
          _self.moveX >=
          document.body.offsetWidth - e.target.clientWidth
        ) {
          _self.moveX = document.body.offsetWidth - e.target.clientWidth;
        }
        if (_self.moveY < 0) {
          _self.moveY = 0;
        } else if (
          _self.moveY >=
          document.body.offsetHeight - e.target.clientWidth
        ) {
          // 要想在移动时，下方可以一直移动到浏览器下边框，需要最外面的div的高度
          _self.moveY = document.body.offsetHeight - e.target.clientWidth;
        }
